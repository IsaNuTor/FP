...
void colocarMensajesBuzonEnLosChats(tDatosCliente &cliente, tServidor &servidor, string usuario)
{
	...
	for(int i = 0; i < mensajes_sin_leer.cont_mensajes; i++)
	{
		...
		if(pos_chat == -1)
		{
			// Creamos la lista de Chats con 10 elementos
			if(cliente.listaChatsCliente.cont_chats == 0)
			{
				cliente.listaChatsCliente.arrayChats= new tChat[MAX_CHATS];
			}
			...
			anadirChatAlFinalDeLaLista(cliente.listaChatsCliente, chat);
		}
		else
		{
...
void ejecutarApp(tDatosCliente &cliente, tServidor &servidor, string idUsuario)
{
	...
	int numero;
	bool salir = false, ok = false, salirChat, textoIntroducido;
		...
		case 'c':
			{
				...
				if(numero < cliente.listaChatsCliente.cont_chats) // Ese chat no existe, notificamos el error.
				{
					...
					while(!salirChat)
					{
						salirChat = gestionarChat(cliente.listaChatsCliente.arrayChats[numero], servidor, textoIntroducido);
						system("cls");
					}
					if(textoIntroducido)
					{
